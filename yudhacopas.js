...
const starts = async (dha = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('Katashi BOTZ V2', {
		font: 'Standard',
	...
	// Menunggu QR
	dha.on('qr', () => {
		console.log(color('[', 'pink'), color('!', 'red'), color(']', 'pink'), color('HEI KONTOL CEPAT SCAN LU OWNER GUA SEKARANG'))
	})

	// Menghubungkan
	fs.existsSync(`./${setting.sessionName}.json`) && dha.loadAuthInfo(`./${setting.sessionName}.json`)
	dha.on('connecting', () => {
		console.log(color('[ Katashi ]', 'purple'), color('PROSES PENYAMBUNGAN'));
	})
const spinner = { 
  "interval": 120,
  "frames": [
